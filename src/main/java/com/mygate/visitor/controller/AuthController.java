...
import com.mygate.visitor.entity.HR;
import com.mygate.visitor.dto.UserResponseDTO;
...
import com.mygate.visitor.service.EmailService;
...
public class AuthController {
    ...
    public ResponseEntity<?> sendStudentOTP(@RequestBody Map<String, String> request) {
        try {
            ...
            response.put("devOtp", otp); // visible in dev/terminal
    ...
    public ResponseEntity<?> sendSecurityOTP(@RequestBody Map<String, String> request) {
        try {
            // Accept both 'securityId' and 'securityCode' for compatibility
            String securityCode = sanitizeInput(request.get("securityId") != null
                    ? request.get("securityId") : request.get("securityCode"));
            ...
            String otp = generateOTP();
            String hashedOTP = passwordEncoder.encode(otp);
            otpStore.put(email, hashedOTP);
            otpTimestamp.put(email, System.currentTimeMillis());

            // Log OTP to console
            System.out.println("\n" + "=".repeat(70));
            System.out.println("🔐 OTP GENERATED - SECURITY LOGIN");
            System.out.println("=".repeat(70));
            System.out.println("│ Security ID : " + security.getSecurityId());
            System.out.println("│ Name        : " + security.getName());
