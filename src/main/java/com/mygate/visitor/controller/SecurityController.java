...
import com.mygate.visitor.entity.ScanLog;
import com.mygate.visitor.entity.Staff;
import com.mygate.visitor.entity.Student;
import com.mygate.visitor.entity.StaffMember;
import com.mygate.visitor.entity.QRTable;
...
import com.mygate.visitor.entity.HOD;
import com.mygate.visitor.entity.Department;
import com.mygate.visitor.entity.Visitor;
...
import com.mygate.visitor.repository.StaffMemberRepository;
import com.mygate.visitor.repository.QRTableRepository;
import com.mygate.visitor.repository.HODRepository;
import com.mygate.visitor.repository.DepartmentRepository;
import com.mygate.visitor.repository.VisitorRepository;
import com.mygate.visitor.service.NotificationService;
...
public class SecurityController {
    ...
    @Autowired
    private com.mygate.visitor.service.GroupPassService groupPassService;
    
    @Autowired
    private com.mygate.visitor.service.GroupPassQRGenerationService groupPassQRGenerationService;
    
    @Autowired
    private com.mygate.visitor.repository.RailwayExitLogRepository railwayExitLogRepository;
    
    @Autowired
    private com.mygate.visitor.repository.RailwayEntryRepository railwayEntryRepository;
    
    @Autowired
    private com.mygate.visitor.repository.SecurityPersonnelRepository securityPersonnelRepository;
    
    @Autowired
    private com.mygate.visitor.repository.GatePassRequestRepository gatePassRequestRepository;
    
    @Autowired
    private com.mygate.visitor.service.LateEntryService lateEntryService;
    
    @Autowired
    private com.mygate.visitor.service.EmailService emailService;
    
    @Autowired
    private com.mygate.visitor.service.UnifiedVisitorService unifiedVisitorService;
    ...
    private boolean isValidQRFormat(String qrCode) {
        ...
        String userType = parts[0].toUpperCase();
        return userType.equals("ST") || userType.equals("SF") || userType.equals("VG") || userType.equals("GP");
    }
    ...
    public ResponseEntity<?> scanLateEntry(@RequestBody java.util.Map<String, String> request) {
        try {
            ...
            if (studentOpt.isPresent()) {
                com.mygate.visitor.service.LateEntryService.LateEntryResponse response = 
        ...
        try {
            ...
            if (!qrTableOpt.isPresent()) {
                ...
                PersonType personType = userType.equals("ST") ? PersonType.STUDENT : 
                                       userType.equals("SF") ? PersonType.FACULTY : PersonType.VISITOR;
            ...
            if (userType.equals("ST") || userType.equals("SF")) {
                // ST/SF: Only use exit column
                
                // Validate: ST/SF should never have entry value
                if (qrTable.getEntry() != null) {
                    System.out.println("❌ ACCESS DENIED - ST/SF should not have entry value: " + qrCode);
                    
                    PersonType personType = userType.equals("ST") ? PersonType.STUDENT : PersonType.FACULTY;
                    
                    // Do NOT save invalid QR codes to scan history
                    return ResponseEntity.status(403).body(new java.util.HashMap<String, Object>() {{
                        put("qrCode", qrCode);
                        put("status", "DENIED");
                        put("message", "Invalid QR configuration for ST/SF");
                        put("accessGranted", false);
                ...
                if (randomNumber != null && randomNumber.equals(qrTable.getExit())) {
                    ...
                    exitLog.setUserType(userType.equals("ST") ? "STUDENT" : "STAFF");
                    ...
                    System.out.println("✅ ST/SF EXIT APPROVED - Random matched exit column, deleted row: " + qrCode);
                } else {
                ...
                if (qrTable.getEntry() != null && qrTable.getExit() == null) {
                    ...
                    if (randomNumber != null && randomNumber.equals(qrTable.getEntry())) {
                        ...
                        // Create entry record in Entry table BEFORE moving random
                        RailwayEntry entryRecord = new RailwayEntry();
                        entryRecord.setQrId(qrTable.getId());
                        entryRecord.setUserId(userId);
                        entryRecord.setUserType("VISITOR");
                        entryRecord.setScannedBy("Security Guard");
                        entryRecord.setScanLocation(scanLocation);
                        entryRecord.setTimestamp(java.time.LocalDateTime.now());
                        railwayEntryRepository.save(entryRecord);
                        
                        System.out.println("✅ VG ENTRY record created in Entry table");
            ...
            System.out.println("✅ ACCESS GRANTED - Scan log created for: " + personName);
    ...
    public ResponseEntity<List<PersonToMeetDTO>> getStaffAndHODsByDepartment(@PathVariable String departmentName) {
        try {
            ...
            if (departmentName.length() > 5) {
                ...
                List<com.mygate.visitor.entity.Department> depts = departmentRepository.findAll();
                for (com.mygate.visitor.entity.Department dept : depts) {
                ...
                try {
                    com.mygate.visitor.entity.Department dept = departmentRepository.findById(departmentName).orElse(null);
                    if (dept != null) {
                        deptFullName = dept.getName();
                    }
                } catch (Exception e) {
        ...
        try {
            ...
            for (java.util.List<ScanLog> personScans : scansByPerson.values()) {
                ...
                if (lastScan.getScanLocation() != null && 
                    lastScan.getScanLocation().toLowerCase().contains("exit")) {
                    status = "EXITED";
                    outTime = lastScan.getScanTime().format(java.time.format.DateTimeFormatter.ISO_DATE_TIME);
                ...
                scanData.put("id", firstScan.getStudentId() != null ? firstScan.getStudentId() : 
                                   (firstScan.getFacultyId() != null ? firstScan.getFacultyId() : firstScan.getId()));
                scanData.put("name", firstScan.getPersonName());
                scanData.put("type", firstScan.getPersonType() != null ? firstScan.getPersonType().toString() : "UNKNOWN");
                scanData.put("entryTime", firstScan.getScanTime().format(java.time.format.DateTimeFormatter.ISO_DATE_TIME));
    ...
    public ResponseEntity<?> generateGroupPass(@RequestBody com.mygate.visitor.service.GroupPassQRGenerationService.GroupPassRequest request) {
        try {
    ...
    public ResponseEntity<?> getSecurityProfile(@PathVariable String securityId) {
        try {
            ...
            Optional<com.mygate.visitor.entity.SecurityPersonnel> securityOpt =
            ...
            com.mygate.visitor.entity.SecurityPersonnel security = securityOpt.get();
    ...
    public ResponseEntity<List<java.util.Map<String, Object>>> getRecentScans(
        ...
        try {
            ...
            // Filter valid scans and sort by scan time (newest first)
            List<ScanLog> recentScans = allScans.stream()
                .filter(scan -> scan.getAccessGranted() && scan.getScanTime() != null)
            ...
            for (ScanLog scan : recentScans) {
                ...
                scanData.put("personName", scan.getPersonName());
                scanData.put("personType", scan.getPersonType() != null ? scan.getPersonType().toString() : "UNKNOWN");
                scanData.put("purpose", scan.getPurpose() != null ? scan.getPurpose() : "General");
                scanData.put("scanTime", scan.getScanTime().format(java.time.format.DateTimeFormatter.ISO_DATE_TIME));
    ...
    public ResponseEntity<?> getMyVisitorRequests(@RequestParam String securityId) {
        try {
            ...
            // Find all visitors registered by this security personnel
            List<Visitor> visitors = visitorRepository.findAll().stream()
            ...
            List<java.util.Map<String, Object>> visitorData = visitors.stream()
                .map(v -> {
