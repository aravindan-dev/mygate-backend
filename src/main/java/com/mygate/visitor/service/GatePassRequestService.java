...
public class GatePassRequestService {
    ...
    // Approve by staff
    @Transactional
    public GatePassRequest approveByStaff(Long requestId, String staffCode) {
    ...
    // Approve by HOD
    @Transactional
    public GatePassRequest approveByHOD(Long requestId, String hodCode) {
        ...
        if ("STUDENT".equals(saved.getUserType())) {
            ...
            // If it's a bulk pass with SEG (Staff Excluded Group), notify the receiver
            if ("BULK".equals(saved.getPassType()) && !saved.getIncludeStaff()) {
                notificationService.notifyBulkPassReceiver(saved);
            }
        }
    ...
    public void generateSinglePassQRCode(GatePassRequest request) {
        try {
            ...
            if (!existingQRs.isEmpty()) {
                ...
                // Check if existing QR uses correct format (SF|, ST|, or VG|)
                if (existingQR != null && 
                    (existingQR.startsWith("SF|") || existingQR.startsWith("ST|") || existingQR.startsWith("VG|"))) {
            ...
            if (request.getUserType() != null) {
                if (request.getUserType().equals("STAFF")) {
                    ...
                    userTypePrefix = "VG";
                    userId = "null"; // Visitors have null as ID
                }
            }
            ...
            qrTable.setUserType(userTypePrefix); // SF for Staff, ST for Student, VG for Visitor
